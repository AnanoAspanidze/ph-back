...
<div class="section__theme__container padding-0">
        @php
    ...
    @if($topic->resources)
      <div class="d-flex justify-content-center">
          <div class="paging__container sm">
            ...
            @foreach($topic->resources as $index => $item)
              <a href="{{route('topics.inner', [$topic->id, $item->id])}}" class="paging__item {{ $item->id === $resource->id ? 'active' : ''}} {{class_basename($item->resourceable_type) == 'Step' ? 'green' : ''}} ">{{$index + 1}}</a>
            @endforeach
