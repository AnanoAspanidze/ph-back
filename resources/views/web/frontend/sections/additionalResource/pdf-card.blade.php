<div class="col-12 col-md-6 col-lg-4 col-xxl-3 resource__item__container">
    <div class="resource__item">
        <div class="resource__item-relative">            
            @if($resource->image)
                <img src="{{asset($resource->getUrlPath().'/images/'.$resource->image)}}" alt="resource image" class="resource__item__img file active"
                        data-bs-toggle="modal" data-bs-target="#fullscreenImgModal" />
            @else
                <a href="{{asset($resource->getUrlPath().'/files/'.$resource->pdf)}}" target="_blank" class="w-100 h-100 d-flex justify-content-center align-items-center">
                    <img src="{{asset('img/icons/file.svg')}}" alt="resource image" class="resource__item__img file" />
                </a>
            @endif
            <div class="resource__item__icon__position">
                <div class="resource__item__icon__container">
                    <a href="{{asset($resource->getUrlPath().'/files/'.$resource->pdf)}}" target="_blank">
                        <div class="resource__item__icon__bg">
                            <img src="{{asset('img/icons/file.svg')}}" alt="icon" class="resource__item__icon">
                        </div>
                    </a>
                    <div class="resource__item__icon__bg">
                        <a href="{{asset($resource->getUrlPath().'/files/'.$resource->pdf)}}" download>
        ...
        <a href="{{asset($resource->getUrlPath().'/files/'.$resource->pdf)}}" target="_blank" class="resource__item-link">
