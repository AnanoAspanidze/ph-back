...
@section('title', 'გვერდის დამატება')

@section('content_header')
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-12 col-md-10">
                <h4 class="m-0">გვერდის დამატება</h4>
            </div>
        </div>
    </div>
@endsection
...
<div class="container-fluid">
    <div class="row">
        <div class="col-12 col-md-10 m-auto">
            ...
            <form role="form" method="post" action="{{route('other_page.store', $topic->id)}}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="card card-primary">
                <div class="card-body">
                    ...
                    <input type="checkbox" class="form-check-input" @if(old('type') || (old('resource') && old('resource')['layout'])) checked="checked"  @endif id="page-has-main-visual">
                    <label class="form-check-label" for="exampleCheck1">აქვს თუ არა გვერდს მთავარი
                        ვიზუალი?</label>
                    </div>

                    <div id="main-visual-options" style="display: {{ (old('type') || (old('resource') && old('resource')['layout']) ) ? 'block' : 'none'  }}">
                        <div class="form-group">
                            ...
                            <select class="form-control" id="layout" name="resources[layout]">
                                ...
                                @foreach(config('studentpages.layouts') as $key => $value)
                                    <option value="{{$key}}" @if (old('resources') && old('resources')['layout'] == $key) selected="selected" @endif >{{$value}}</option>
                                @endforeach
                            </select>
                    ...
                    <div id="upload-video-wrapper" style="display: {{ old('type') === 'video'  ? 'block' : 'none'  }}">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="videoURL"  value="{{old('link')}}" placeholder="შეიყვანეთ ლინკი">
                            @error('link')
                                <span class="help-error-block">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="form-check form-group">
                        <input type="checkbox" name="resources[show_steps]" class="form-check-input">
